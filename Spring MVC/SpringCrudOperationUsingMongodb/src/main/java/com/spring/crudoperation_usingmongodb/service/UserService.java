package com.spring.crudoperation_usingmongodb.service;
...
import com.spring.crudoperation_usingmongodb.factory.MongoFactory;
import com.spring.crudoperation_usingmongodb.user.User;

@Service("userService")
@Transactional
public class UserService {

	static String db_name = "Syamdb", db_collection = "employee";
	private static Logger log = Logger.getLogger(UserService.class);

	// Fetch all users from the mongo database.
	public List getAll() {
		List user_list = new ArrayList();
		DBCollection coll = MongoFactory.getCollection(db_name, db_collection);// driver
		...
		while (cursor.hasNext()) {
			...
			user.setMobileNumber(dbObject.get("mobileNumber").toString());
			user.setEmailId(dbObject.get("emailId").toString());
			user.setGender(dbObject.get("gender").toString());
			user.setDOB(dbObject.get("DOB").toString());
			user.setJoiningDate(dbObject.get("JoiningDate").toString());
			user.setDepartment(dbObject.get("department").toString());
			user.setSalary(dbObject.get("salary").toString());
	...
	public Boolean add(User user) {
		...
		try {
			...
			doc.put("mobileNumber", user.getMobileNumber());
			doc.put("emailId", user.getEmailId());
			doc.put("gender", user.getGender());
			doc.put("DOB", user.getDOB());
			doc.put("JoiningDate", user.getJoiningDate());
	...
	public Boolean edit(User user) {
		...
		try {
			...
			edited.put("mobileNumber", user.getMobileNumber());
			edited.put("emailId", user.getEmailId());
			edited.put("gender", user.getGender());
			edited.put("DOB", user.getDOB());
			edited.put("JoiningDate", user.getJoiningDate());
			edited.put("department", user.getDepartment());
			edited.put("salary", user.getSalary());
	...
	public User findUserId(String id) {
		...
		u.setMobileNumber(dbo.get("mobileNumber").toString());
		u.setEmailId(dbo.get("emailId").toString());
		u.setGender(dbo.get("gender").toString());
		u.setDOB(dbo.get("DOB").toString());
		u.setJoiningDate(dbo.get("JoiningDate").toString());
		u.setDepartment(dbo.get("department").toString());
		u.setSalary(dbo.get("salary").toString());
